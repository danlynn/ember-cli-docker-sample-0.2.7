...
function npm() {
	if [[ $PWD/ = $PREV_ROOT_DIR/* ]]; then
		...
		$PREV_ROOT_DIR/project.rb docker_exec npm $@
	else
		# echo "`which npm` $@"
		`which npm` $@
	fi
...
# setup docker env vars - but only if the docker-machine is running
[ $(./project.rb vm_running) ] && eval "$(docker-machine env `./project.rb config docker-machine name`)"
...
# TODO: Maybe switch these over from 'run' to 'exec'?  We would need the container ID tho.
